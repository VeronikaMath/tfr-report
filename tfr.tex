...
Since the solution of the primal equation depends on the solution of the dual equation and vice versa, we will need to take an iterative approach, numerically solving the primal equation first, using that numerical solution as input into the numerical scheme for the dual equation, etc. We recall that we are interested in the final solution $u$ to the primal equation, but not directly in the final solution $z$ to the dual equation. The overall structure of the algorithm will be as follows:

\begin{enumerate}
    \item Initialize $u$ as $u_0(x,v)$ and solve the equation for $u$ with no source term, forward in time up to $t=T$.
...
\begin{align} % or should this be z^{n-1}?
    \frac{u^n-u^{n-1}}{\Delta t} + \div{(\underline{b}u^{n-1} - A \nabla u^n)} = \frac{1}{\alpha} z^n,
\end{align}
%
...
\end{align}
%
and similarly defining $\underline{u}^{n-1}$ and $\underline{z}^n$. The matrix form of \autoref{eq:finite-difference} then becomes 
%
\begin{align} 
    \label{eq:fd-matrix}
    B \underline{u}^n = C \underline{u}^{n-1} + \Delta t \underline{z}^n,
\end{align}
%
where $B$ and $C$ are matrices of size $M^2 \cross M^2$, and $\underline{u}^n$, $\underline{u}^{n-1}$ and $\underline{z}^n$ are column vectors of length $M^2$. Enforcing zero boundary conditions, the nonzero entries of $B$ are given by
%
\begin{align} 
    \begin{cases} 
...
\end{align}
%
and the nonzero entries of $C$ are given by 
%
\begin{align} 
    \begin{cases}
...
We note that the rows (and columns) of $B$ and $C$ are labelled in the same order as the rows of $\underline{u}^n$, namely $(1,1),(1,2),\dots,(1,M),(2,1),\dots(M,M)$. We map this labeling to the index $k$ running from $1$ to $M^2$ as 
%
\begin{align}
    k(i,j) = (i-1)M + j.
\end{align}
...
By inverting the matrix $B$ in \autoref{eq:fd-matrix} we get an explicit expression for $\underline{u}^n$ in terms of $\underline{u}^{n-1}$, namely
%
\begin{align} 
    \underline{u}^n = B^{-1}C\underline{u}^{n-1} + \Delta t B^{-1}\underline{z}^n.
\end{align}
%
This equation gives the procedure for stepping one step forward in time, so to get from $t=0$ to $T=0$, taking $n$ steps of size $\Delta t = \frac{T}{N}$ in time, we will need to apply the equation $N$ times.
